...
async def create_vaccination(
    ...
    try:
        data = jsonable_encoder(vaccination.dict())
        ...
        existing = supabase_admin.table("vaccinations") \
            ...
            .eq("vaccine_name", vaccination.vaccination_name if hasattr(vaccination, 'vaccination_name') else vaccination.vaccine_name) \
            .execute()
...
@router.get("/milestones/{child_id}", response_model=List[dict])
async def get_child_milestones(
    ...
    try:
        # Check cache
        cache_key = f"postnatal:milestones:{child_id}"
        if CACHE_AVAILABLE and cache:
            cached_data = cache.get(cache_key)
            if cached_data:
                cached_data["cached"] = True
        ...
        query = supabase_admin.table("milestones") \
            .select("*") \
            .eq("child_id", child_id) \
            .eq("is_achieved", True) \
        ...
        # Cache
        if CACHE_AVAILABLE and cache:
            cache.set(cache_key, milestones, ttl_seconds=60)
...
@router.post("/milestone/toggle", status_code=status.HTTP_200_OK)
async def toggle_milestone(
    request: MilestoneToggleRequest,
    current_user: dict = Depends(get_current_user)
):
    """Toggle a milestone achievement status"""
    try:
        # Check if exists
        existing = supabase_admin.table("milestones") \
            .select("*") \
            .eq("child_id", request.child_id) \
            .eq("milestone_name", request.milestone_name) \
            .single() \
            .execute()
        ...
        if record:
            # Toggle
            new_status = not record.get("is_achieved", False)
            supabase_admin.table("milestones") \
                .update({
                    "is_achieved": new_status,
                    "achieved_date": datetime.utcnow().isoformat().split('T')[0] if new_status else None
                }) \
                .eq("id", record['id']) \
                .execute()
        else:
            # Create new
            category_map = {
                'Motor': 'gross_motor', 'Fine Motor': 'fine_motor', 
                'Language': 'language', 'Cognitive': 'cognitive', 
                'Social': 'social_emotional', 'Sensory': 'cognitive', 
                'Self Care': 'self_care'
            }
            db_category = category_map.get(request.milestone_category, 'gross_motor')
            ...
            supabase_admin.table("milestones").insert({
                ...
                "is_achieved": True,
                "achieved_date": datetime.utcnow().isoformat().split('T')[0],
        ...
        # Invalidate cache
        if CACHE_AVAILABLE and cache:
            cache.delete(f"postnatal:milestones:{request.child_id}")
